...
//  Theo
//
//  Created by Alexis Muñoz on 1/15/23.
//
...
class BodySkeleton: Entity {
    ...
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let jointFromEntityOffsetFromRoot = simd_make_float3(fromJointEntityTransform.columns.3) //relative to hipjoint
        
        let jointFromEntityPosition = jointFromEntityOffsetFromRoot + rootPosition //relative to a reference frame
        
        let jointToEntity0ffsetFromRoot = simd_make_float3(toJointEntityTransform.columns.3) // relative to hipjoint
        
        let jointToEntityPosition = jointToEntity0ffsetFromRoot + rootPosition // relative to world reference frame
    ...
    private func createBoneEntity(for skeletonBone: SkeletonBone, diameter: Float = 0.01, color: UIColor = .white) ->
    Entity {
